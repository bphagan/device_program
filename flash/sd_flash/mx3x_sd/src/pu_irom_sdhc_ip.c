...
static U32 sdhc_check_response(void)
{
...
#ifdef TY_DBG
	{
		...
		U8 au8StrBuf[100] = { 0 };
		sprintf(au8StrBuf, "Ty, chk_resp: sdhc_status: %d, judge: %d result: %d\n", psdhc->sdhc_status, ((psdhc->sdhc_status & SDHC_STATUS_END_CMD_RESP_MSK) &&
	!(psdhc->sdhc_status & SDHC_STATUS_TIME_OUT_RESP_MSK) &&
	!(psdhc->sdhc_status & SDHC_STATUS_RESP_CRC_ERR_MSK)), sdhc_status);
        atk_channel_send(au8StrBuf, sizeof(au8StrBuf));
        memset(au8StrBuf, 0, 100);
        sprintf(au8StrBuf, "Ty, 1:%d 2:%d 3:%d", psdhc->sdhc_status & SDHC_STATUS_END_CMD_RESP_MSK, psdhc->sdhc_status & SDHC_STATUS_TIME_OUT_RESP_MSK, psdhc->sdhc_status & SDHC_STATUS_RESP_CRC_ERR_MSK);
		atk_channel_send(au8StrBuf, sizeof(au8StrBuf));
	}
#endif
...
void interface_configure_clock(sdhc_freq_t frequency)
{
    ...
    if ( ( (*(VP_U32)CCM_CCMR ) & CLOCK_SEL_MASK ) == CKIL_CLOCK_SEL)
    {
        ...
        if(frequency == IDENTIFICATION_FREQ)
        {
            ...
            psdhc->sdhc_clk_rate |= (PRE_CLK_DIV_64 << SDHC_CLK_RATE_PRESCALER_SHIFT) | CLK_DIV_3;
