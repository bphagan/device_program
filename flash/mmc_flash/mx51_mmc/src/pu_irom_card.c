...
==========================================================================*/
UINT32 address_mode;
UINT32 Card_rca = ONE;                   /* Relative Card Address */
...
UINT32 card_init(UINT32 bus_width, UINT32 base_address)
{
    ...
    init_status = interface_reset(ESDHC_ONE_BIT_SUPPORT,
                                  ESDHC_LITTLE_ENDIAN_MODE);
    if(init_status == FAIL)
    {
        return init_status;
    }
    ...
    /* Send 80 SD clock to card */
    interface_initialization_active();

    /* Issue Software Reset to card */
	init_status= card_software_reset(); 
	...
	if (init_status == SUCCESS)
	{
        ...
        {
#if defined (REV_TO1)
			/* set enainess and bus width for mmc*/
			interface_set_bus_width_endianess(ESDHC_ONE_BIT_SUPPORT, ESDHC_LITTLE_ENDIAN_MODE);
#elif defined (REV_TO2)
                /* Set strong pull-up for the CMD line */
                esdhc_set_cmd_pullup(STRONG);
#endif
...
{
    command_t cmd;
	UINT32 card_state = ZERO;
	UINT32 transfer_status = ZERO;
    command_response_t response;
	...
	card_command_config(&cmd,CMD7,card_address,READ,RESPONSE_48_CHECK_BUSY,
...
/*==========================================================================
FUNCTION:  void card_command_config (command_t *cmd_config,
                                     UINT32 index,
                                     UINT32 argument,
                                     xfer_type_t transfer,
                                     response_format_t format,
                                     data_present_select data,
                                     crc_check_enable crc,
                                     cmdindex_check_enable cmdindex)
DESCRIPTION:
   Configure the command paramteres for card.

ARGUMENTS PASSED:
   command_t *cmd_config
   UINT32 index
   UINT32 argument
   xfer_type_t transfer
   response_format_t format
...
==============================================================================*/
void card_command_config(command_t *cmd_config,
                         UINT32 index,
                         UINT32 argument,
                         xfer_type_t transfer,
                         response_format_t format,
...
FUNCTION: UINT32 card_send_virtual_read_command(void)

DESCRIPTION:
   This function sends virtual read command to eSDHC controller.
   This read command is not routed to device.
...
IMPORTANT NOTES:
    1. Clear interrupt.
    2. configure the block count to 4, WML = 128.
    3. Send vertual read command, CMD18 to read first 2K boot data.
==================================================================================================*/
UINT32 card_send_virtual_read_command(void)
{
    command_t cmd;
    UINT32 status = FAIL;
    UINT32 num_block = (TWO_K_SIZE/ESDHC_BLK_LEN);
    ...
    /* Configure virtual CMD18 for 2K data read */
    card_command_config(&cmd, CMD18,
                              ZERO,
                              READ,
                              RESPONSE_NONE,
                              DATA_PRESENT,
