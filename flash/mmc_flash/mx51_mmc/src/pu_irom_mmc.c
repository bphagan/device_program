...
static UINT32 mmc_send_switch_command(UINT32 switch_arg);
static UINT32 emmc_get_boot_ack (void);
...
/*==========================================================================
FUNCTION: static UINT32 mmc_get_csd(void)
DESCRIPTION:
...
Detailed Description:
  ...
  3. Put the CSD field values in to mmc_csd.
==============================================================================*/
static UINT32 mmc_get_csd(void)
{
    command_t cmd;
    ...
    else
    {
        ...
        mmc_csd.r2w_factor = MMC_CSD_EXTRACT(response.cmd_rsp3,
