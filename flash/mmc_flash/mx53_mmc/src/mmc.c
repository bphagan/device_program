...
#define __be32_to_cpu(x) ___swab32(x)
...
int
mmc_erase(int dev_num, ulong start, ulong end)
{
	struct mmc_cmd cmd;
	int err;
	struct mmc *mmc = find_mmc_device(dev_num);
	...
	cmd.resp_type = MMC_RSP_R1;
	cmd.flags = 0;
	cmd.cmdidx = IS_SD(mmc) ? SD_CMD_ERASE_START : MMC_CMD_ERASE_START;
	...
	if (err) {
		...
		cmd.cmdarg = end * blklen;

        cmd.resp_type = MMC_RSP_R1;
	cmd.flags = 0;
	cmd.cmdidx = IS_SD(mmc) ? SD_CMD_ERASE_END : MMC_CMD_ERASE_END;
...
int mmc_initialize(struct mmc **pmmc)
{
