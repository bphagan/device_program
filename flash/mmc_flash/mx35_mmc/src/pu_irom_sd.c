        ...
        while (len > 0 && !read_block_status) 
        {
            ...
            else
            {
#ifdef TY_DBG
				{
					// Terry adds for debug, :-)
					U8 au8StrBuf[100] = { 0 };
					sprintf(au8StrBuf, "Ty, Issue CMD17 Succeed!\n");
					atk_channel_send(au8StrBuf, sizeof(au8StrBuf));
				}
#endif	
...
#ifdef TY_DBG
				{
					// Terry adds for debug, :-)
					U8 au8StrBuf[100] = { 0 };
					sprintf(au8StrBuf, "Ty, read_block_status: %d\n", read_block_status);
					atk_channel_send(au8StrBuf, sizeof(au8StrBuf));
				}
#endif
...
UINT32 sd_data_write (UINT32 *dest_ptr, UINT32 length, UINT32 offset, response_callback callback)
{
    ...
    else
    {
#ifdef TY_DBG
        {
            // Terry adds for debug, :-)
            U8 au8StrBuf[100] = { 0 };
            sprintf(au8StrBuf, "Ty, issue CMD16 Succeed!\n");
            atk_channel_send(au8StrBuf, sizeof(au8StrBuf));
        }
#endif
...
#ifdef TY_DBG
        {
            // Terry adds for debug, :-)
            U8 au8StrBuf[100] = { 0 };
            sprintf(au8StrBuf, "Ty, issue CMD24 Succeed!\n");
            atk_channel_send(au8StrBuf, sizeof(au8StrBuf));
        }
#endif
...
static UINT32 sd_get_rca (void)
{
    ...
    else
    {
        ...
        Card_rca = ((UINT32) (response.cmd_rsp0 >> CARD_RCA_SHIFT));
