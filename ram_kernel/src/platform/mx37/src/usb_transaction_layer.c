...
-------------------------   ------------    ----------   -------------------------------------------
Radhish                     09/07/08       ENGcm06491    Missing break statement in tl_init fn 
Siva Pothireddy             06/05/2008     ENGcm05549  STALL Fix for Marley
Siva Pothireddy                 13/mar/2008   ENGcm03786    Bytes are reversed whille filling end point zero buffer                  
Prabhat Singh                08/oct/2007    engcm00737       USB device detect reset
...
static usb_state_t g_usb_dev_state = USB_DEV_DUMMY_STATE; /* State os USB Device */
...
BOOL tl_check_for_bus_activity(void)
{
    BOOL rtn = FALSE;
    /* Check if Bus Reset Received */
    if(ipl_check_usb_device_reset() == TRUE)
    {
        rtn = TRUE;
    }
